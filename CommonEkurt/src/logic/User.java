...
public class User implements Serializable {
	 ...
	 * @param role
	 */
	public User(int id, String username, String password, boolean isLoggedIn, String firstName, String lastName,
			String email, String telephone, RoleEnum role, StatusEnum status, RegionEnum region) {
		this.id = id;
		...
		try {
			while (rs.next()) {
				users.add(new User(rs.getInt("id"), rs.getString("username"), rs.getString("password"),
						rs.getInt("is_logged_in") == 1, rs.getString("firstname"), rs.getString("lastname"),

...
public class DeliveryOperatorQuaries {
			...
			try {
				ResultSet rs = dbController.getInstance().executeQuery(
						"SELECT order_code, client_id, order_date, supply_date, total_price,order_status FROM ekurt.orders WHERE region='"
								+ region.toString() + "' AND supply_method='" + method + "'");
				if (!(rs.next())) {
				...
				while (rs.next()) {
					...
					String enumValue = rs.getString("order_status");
	...
	if (msg instanceof Transaction) {
		...
		int rowsAffected = dbController.getInstance().executeUpdate(
                "UPDATE orders SET order_status = 'ON_THE_WAY' WHERE "
                + "order_code ='"+obj.getOrderId()+"' AND order_status = 'WAIT_APPROVAL'");
		if (rowsAffected ==1) {
	...
	public static void CompleteOrder(Transaction msg) {
		if (msg instanceof Transaction) {
			DeliveryOrder obj = (DeliveryOrder) msg.getData();
			int rowsAffected = dbController.getInstance().executeUpdate(
	                "UPDATE orders SET order_status = 'COMPLETE' WHERE "
	                + "order_code ='"+obj.getOrderId()+"' AND order_status = 'RECEIVED'");
			if (rowsAffected ==1) {
